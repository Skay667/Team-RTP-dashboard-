...
let laps = [
  { car: 'Porsche 911 Turbo S',        time: '1:48.254', category: 'Police' },
  { car: 'BMW M4',                     time: '1:52.159', category: 'Police' },
  { car: 'Dodge Charger SRT Hellcat',  time: '1:53.827', category: 'Police' },
  { car: 'Toyota Crown Athlete G',     time: '1:55.464', category: 'Police' },
  { car: 'Nissan GT-R (non préparée)', time: '1:57.519', category: 'Police' },
  { car: 'Renault Mégane RS',          time: '1:59.000', category: 'Police' },
];
...
const VOTE_STATE_KEY = 'teamrtp_vote_state';
const VOTE_LOCK_KEY = 'teamrtp_last_vote_ts';
const VOTE_COOLDOWN_MS = 7 * 24 * 60 * 60 * 1000; // 1 semaine
...
// Lecture protégée : certains navigateurs (mode privé strict, aperçu en
// iframe, etc.) peuvent bloquer localStorage — le site continue de
// fonctionner en mémoire simple dans ce cas.
function loadVoteState() {
  try {
    const raw = localStorage.getItem(VOTE_STATE_KEY);
...
function saveVoteState() {
  try {
    localStorage.setItem(VOTE_STATE_KEY, JSON.stringify(voteState));
  } catch (e) { /* stockage indisponible */ }
}

const voteState = { ...defaultVoteState };
const storedVoteState = loadVoteState();
if (storedVoteState) {
  Object.keys(defaultVoteState).forEach((member) => {
    if (typeof storedVoteState[member] === 'number') {
      voteState[member] = storedVoteState[member];
    }
  });
}
...
function getFilteredLaps() {
  return laps.filter((lap) => {
    const matchesFilter = currentFilter === 'Tout' || lap.category === currentFilter;
...
function getLastVoteTimestamp() {
  try {
    const raw = localStorage.getItem(VOTE_LOCK_KEY);
...
function setLastVoteTimestamp(ts) {
  try {
    localStorage.setItem(VOTE_LOCK_KEY, String(ts));
  } catch (e) { /* stockage indisponible */ }
}

function clearLastVoteTimestamp() {
  try {
    localStorage.removeItem(VOTE_LOCK_KEY);
  } catch (e) { /* ignoré */ }
}

// Un seul vote (tous membres confondus) autorisé par appareil toutes les 7 jours
function getVoteLockStatus() {
  const last = getLastVoteTimestamp();
  if (!last) return { locked: false, nextDate: null };
  const elapsed = Date.now() - last;
  if (elapsed >= VOTE_COOLDOWN_MS) return { locked: false, nextDate: null };
  return { locked: true, nextDate: new Date(last + VOTE_COOLDOWN_MS) };
}

function formatVoteDate(date) {
...
function renderVoteList() {
  ...
  Object.keys(voteState).forEach((member) => {
    ...
    row.innerHTML = `
      ...
      <button class="btn btn--vote" data-member="${member}"${lock.locked ? ' disabled' : ''}>Voter</button>
    `;
  ...
  container.querySelectorAll('.btn--vote').forEach((btn) => {
    btn.addEventListener('click', () => {
      if (getVoteLockStatus().locked) return; // sécurité anti double-clic
  ...
  // Message d'état sous la liste (créé une seule fois, mis à jour ensuite)
  let noteEl = document.getElementById('vote-note');
  if (!noteEl) {
    noteEl = document.createElement('p');
    noteEl.id = 'vote-note';
    noteEl.className = 'vote-note';
    container.insertAdjacentElement('afterend', noteEl);
  }
  noteEl.textContent = lock.locked
    ? `Vous avez déjà voté sur cet appareil cette semaine. Prochain vote possible le ${formatVoteDate(lock.nextDate)}.`
    : 'Un vote par semaine et par appareil.';
}

document.getElementById('reset-votes-btn').addEventListener('click', () => {
  Object.keys(voteState).forEach((member) => { voteState[member] = 0; });
  saveVoteState();
  clearLastVoteTimestamp();
  renderVoteList();
});
